...
private:
  ...
  BigRegex(const BigRegex&) { re_assigned = new bool; *re_assigned=false; }  // no X(X&)
  void               operator = (const BigRegex&) { re_assigned = new bool; *re_assigned=false; } // no assignment
...
// BEWARE: a) String::matches(BRXint) returns 1 if the String is empty.
//         b) String::matches(BRXdouble) returns 1 if the String is empty.
//         c) String::matches(BRXdouble) returns 1 if the String is an integer (which does make sense).
