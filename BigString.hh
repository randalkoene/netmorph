...
Modified by Randal A. Koene, 20200425
    see all the notes below relating to friend functions
...
// split string into array res at separators; return number of elements
  ...
  // Update 20200425: New strict C++ rules about friend functions are extremely
  // specific and a bit complex. Default values cannot be given in the friend
  // declaration unless that declaration is also the definition, but then the
  // function would be in class scope instead of global scope. So, the defaults
  // were moved to the outside-of-class-body declaration of the function (see below).
  // See https://docs.microsoft.com/en-us/cpp/cpp/friend-cpp?view=vs-2019
  friend String     common_prefix(const String& x, const String& y, _BIGSTRING_SIZE startpos);
  friend String     common_suffix(const String& x, const String& y, _BIGSTRING_SIZE startpos);
...
// Update 20200425: C++ requires friend declarations not to have default values,
// unless they are also the definition of the function, which we don't want to do
// here, because we don't want it to be just in class scope.
  friend _BIGSTRING_SIZE   readline(istream& s, String& x, char terminator, int discard_terminator);
...
/* Update 20200425: Removing this due to new C++ rules about friend functions.
...
*/
// Yeah, this now had to be here, see all the notes above inside the String class.
String     common_prefix(const String& x, const String& y, _BIGSTRING_SIZE startpos = 0);
String     common_suffix(const String& x, const String& y, _BIGSTRING_SIZE startpos = -1);
/* 
...
// Update 20200425: Due to strict new C++ rules about friend functions the default value must
// now be presented in this declaration (outside of the class body), especially, since we want
// access to readline() in global scope, and we want to define it in BigString.cc.
_BIGSTRING_SIZE   readline(istream& s, String& x, char terminator = '\n', int discard_terminator = 1);
