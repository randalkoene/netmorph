...
#ifndef __cplusplus
	/* Replaced by bool type for C++ compilation */
	typedef char bool;
	#define false 0
	#define true 1
#endif
...
#ifdef __STDC__
void
print_double_string (const char *where, const char *string1, const char *string2,
				int size1, int size2)
#else
void
...
static bool at_begline_loc_p _RE_ARGS ((const char *pattern, const char *p,
					   reg_syntax_t syntax));
static bool at_endline_loc_p _RE_ARGS ((const char *p, const char *pend,
					   reg_syntax_t syntax));
...
static bool group_in_compile_stack _RE_ARGS ((compile_stack_type
						 compile_stack,
						 regnum_t regnum));
...
regex_compile (const char *pattern, size_t size, reg_syntax_t syntax,
			struct re_pattern_buffer *bufp)
#else
static reg_errcode_t
...
#ifdef __STDC__
int
re_search (struct re_pattern_buffer *bufp, const char *string, int size, int startpos, int range,
		struct re_registers *regs)
#else
int
...
static bool group_match_null_string_p _RE_ARGS ((unsigned char **p,
						    unsigned char *end,
						register_info_type *reg_info));
static bool alt_match_null_string_p _RE_ARGS ((unsigned char *p,
						  unsigned char *end,
						register_info_type *reg_info));
static bool common_op_match_null_string_p _RE_ARGS ((unsigned char **p,
							unsigned char *end,
						register_info_type *reg_info));
...
#ifdef __STDC__
int
re_match_2 (struct re_pattern_buffer *bufp, const char *string1, int size1, const char *string2,
...
#ifdef __STDC__
static int
re_match_2_internal (struct re_pattern_buffer *bufp, const char *string1, int size1, const char *string2,
				int size2, int pos, struct re_registers *regs, int stop)
#else
static int
...
bool bnot, same_str_p, best_match_p;
  ...
  for (;;)
	...
	{ /* End of pattern means we might have succeeded.  */
	  ...
	  /* If we haven't matched the entire string, and we want the
		...
		/* *** The problem here may be that the portion below is
		     expected to have same_str_p declared... even if it
			is not used in the region that the jump to the label
			addresses.  */
	...
	case charset_not:
	  {
	    bnot = (re_opcode_t) *(p - 1) == charset_not;
...
#ifdef __STDC__
static int
bcmp_translate (const char *s1, const char *s2, register int len, RE_TRANSLATE_TYPE translate)
#else
static int
...
#ifdef __STDC__
int
regnexec (const regex_t *preg, const char *string, size_t len, size_t nmatch,
