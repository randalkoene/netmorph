...
void devsim_present_HTML_graphical_output(String & statsdatafile, String & netfigfile, String & zoomfigfile, String & abstractfigfile, String & statsfile, bool figsequenceflag, bool combineflag, String & combinetype) {
  ...
  if (outattr_show_figure && figattr_make_full_Fig) if (system(String(FIG2DEV)+" -L pdf "+netfigfile+' '+outputdirectory+"net.pdf")!=0) {
    warning("fig2dev failed.\n");
  }
  if (outattr_show_figure && figattr_make_zoom_Fig) if (system(String(FIG2DEV)+" -L pdf "+zoomfigfile+' '+outputdirectory+"zoom.pdf")!=0) {
    warning("fig2dev failed.\n");
  }
  if (outattr_show_figure && figattr_make_abstract_Fig) if (system(String(FIG2DEV)+" -L pdf "+abstractfigfile+' '+outputdirectory+"abstract.pdf")!=0) {
    warning("fig2dev failed.\n");
  }
...
std::unique_ptr<network> developmental_simulation(Command_Line_Parameters & clp) {
  ...
  report(net->neuron_specific_reports());
  net->setup_completion_requirements();
  report(net->completion.report_completion_criteria());
